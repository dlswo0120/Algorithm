...
public class MergeInvert {
	//도치 세는 변수 count를 전역변수로 지정해 Merge함수를 실행하는 동안 초기화가 되지 않도록 한다.
	static int count = 0;
	...
	public static void MergeSort(int A[], int low, int high) {
		...
		if(low < high) {
			int mid = (low + high) / 2; 
			MergeSort(A, low, mid);   //왼쪽 배열 분할
			MergeSort(A, mid+1, high); //오른쪽 배열 분할
			Merge(A, low, mid, high); //Merge함수 호출
		}
	...
	public static void Merge(int A[], int low, int mid, int high) {
		...
		while(i <= mid && j <= high) {
			...
			if(A[i] >= A[j]) { // 왼쪽 배열의 첫번째 값이 오른쪽 배열보다 더 클 때
				count++; //카운트를 증가 시킨다.	
			    //출력문
				System.out.println("A["+i +"] > A[" +j+"] : "+ A[i] +">"+ A[j]   + "=>  도치! "+"카운트 값: "+ count);
				//i+1부터 j까지 도치가 있는지 찾은 후 있다면 값들을 출력하고 도치와 카운트 값을 출력한다.
				for( int s = i+1 ; s <= j; s++) {
					if(A[s] > A[j]) {
						count++;
						System.out.println("A["+s+"]"+ "> A["+j+"] : "+ A[s] +">"+ A[j] + "=> 도치! "+ "카운트 값: "+ count);
					}
				}	
	...
	public static void main(String[] args) {
		...
		int n = A.length;
		MergeSort(A, 0, n-1);
		System.out.println("총 도치들의 수: " + count); 
	}
